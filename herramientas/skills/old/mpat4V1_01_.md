...
```
VERIFICACION OBLIGATORIA despues de cada carga desde Drive.
No asumir que la carga fue correcta. Verificar explicitamente.

CHEQUEO 1 — El archivo se recibio?
  El contenido descargado tiene mas de 100 caracteres?
  Si no → fallo silencioso. Ver errores abajo.

CHEQUEO 2 — Es la skill correcta?
  El contenido incluye el campo "name:" en el encabezado?
  El valor de "name:" corresponde a la skill que se queria cargar?
  ...
  Si no coincide → carga incorrecta. Ver errores abajo.

CHEQUEO 3 — Tiene las reglas absolutas?
  El contenido incluye al menos una regla con "NUNCA"?
  Si no → archivo corrupto o incompleto. Ver errores abajo.

SI TODOS LOS CHEQUEOS PASAN:
  Informar:
  ┌─────────────────────────────────────────────────────┐
  │ Skill cargada: [nombre del name:]                   │
  │ Alumno:        [ALUMNO_ID]                          │
  │ MCP MPAT4:     activo                               │
  │ Estado:        LISTO — transferiendo control        │
  └─────────────────────────────────────────────────────┘
  Continuar a PASO 5.
...
La skill cargada ejecuta su propio PASO 0 de identificacion
pasando ALUMNO_ID como ya confirmado — no volver a preguntar.

Este skill (skill_alumno) queda en segundo plano.
Solo se reactiva si:
  a) La skill activa pide cambiar a otra skill
  b) El alumno escribe un trigger de una skill distinta a la activa
  c) Ocurre un error de Drive durante la sesion
```
...
Si durante la sesion el alumno escribe un trigger de otra skill:
  ...
  Ejemplo: esta usando SKILL D (produccion) y escribe "investigar tecnologia"

  PROTOCOLO:
    1. Preguntar: "Estas en modo [skill actual]. Queres cambiar a [skill nueva]?
                  El trabajo actual quedara guardado donde esta."
    2. Si confirma:
...
|---|---|---|
| MCP MPAT4 no responde | Servidor no iniciado | Dar instrucciones de inicio (ver PASO 1) |
| Drive no accesible | MCP Drive desconectado | Claude.ai → Herramientas → Google Drive |
| fileId no encontrado | ID desactualizado | Buscar en carpeta 1rrDNblne6P_IwpCnDfmAHKen4KUuua4f |
| Contenido < 100 chars | Error silencioso de Drive | Reintentar la descarga una vez |
| name: no coincide | Se descargo el archivo equivocado | Verificar ID en MAPA DE SKILLS |
| Sin reglas NUNCA | Archivo corrupto o vacio | Avisar al docente: ai.mpat.designer@gmail.com |
| Sin permisos | Alumno sin acceso de lectura | Avisar al docente para otorgar permisos |
...
- Si no entiende el contexto, te pregunta con opciones
- Si el docente actualiza las reglas, las tenes automaticamente
- Si algo falla, este skill te dice exactamente que hacer

No necesitas saber cual skill usar. Solo decir que vas a hacer.
