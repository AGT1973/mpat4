...
## ⛔ NUNCA guardar en formato Google Doc (.gdoc) — SOLO .md, .py o extensión de código
...
## Regla de formato de archivos — INVIOLABLE

```
⛔ NUNCA crear ni guardar archivos en formato Google Doc (.gdoc)
⛔ NUNCA usar mimeType: application/vnd.google-apps.document
⛔ NUNCA omitir disableConversionToGoogleType: true al subir a Drive

✅ SOLO formatos permitidos:
   - .md        → text/plain  + disableConversionToGoogleType: true
   - .py        → text/x-python + disableConversionToGoogleType: true
   - .yaml      → text/plain  + disableConversionToGoogleType: true
   - .json      → application/json + disableConversionToGoogleType: true
   - .toml      → text/plain  + disableConversionToGoogleType: true
   - cualquier extensión de código → text/plain + disableConversionToGoogleType: true

Parámetro obligatorio en TODA llamada create_file a Drive:
...
PASO 5 — GUARDAR EN DRIVE
  ⛔ NUNCA usar format Google Doc — SIEMPRE disableConversionToGoogleType: true
  Guardar ÚNICAMENTE en la carpeta del módulo activo + relay/
  Extensiones válidas: .md · .py · .yaml · .json · .toml · extensiones de código
  Encabezado obligatorio en cada archivo generado:
    Autor: [ALUMNO_ID] · [FECHA]
    Módulo: [módulo] · Versión: V4_02
  ⛔ NUNCA sobreescribir — siempre archivo nuevo con versión
...
| P13 | Formato de archivo | NUNCA .gdoc — SOLO .md / .py / extensión de código |
...
*SKILL.md · mpat4-relay · V4_02 · AGT 2026-05-12*
*Drive raíz: 1gQNxSlp92RpVIfzvyU66czHxLv1sqSqI*
*Cambio V4_02: P13 agregado — NUNCA .gdoc, solo .md / .py / extensión de código*
