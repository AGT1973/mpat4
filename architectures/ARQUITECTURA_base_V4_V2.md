# MPAT4_DEST
# destino: docs
# nombre: ARQUITECTURA_base_V4_V2.md
# alumno: Claude Sonnet 4.6 (agente) — sesion DT-ARQ-01 — 2026-05-29
...
### Version V4_V2: DT-ARQ-01 — P14 + P15 agregados · Claude Sonnet 4.6 · 2026-05-29
...
### Cambios V4_V2 respecto a V4 original (DT-ARQ-01):
### - P14: RBAC ownership agregado (NUEVO V4 — RES.127 / INC-02 / RES.136)
### - P15: Aislamiento de sesion configurable agregado (NUEVO V4 — RES.127 / INC-04 / RES.138)
### - Encabezado actualizado: 13 principios -> 15 principios
### - Fuente de P14/P15: ARQUITECTURA_base_V4_COMPLETA_V4_migrado.md (ariel.garcia.traba · 2026-05-23)
...
## PRINCIPIOS TRANSVERSALES (15 principios — aplican a todas las capas)

Heredados intactos de V3_03 (P1-P13). Ver ARQUITECTURA_base_V3_03.md seccion completa.
P14 y P15 son nuevos en V4 — introducidos via RES.127 (PATCH P14P15).
...
Ningun componente puede invocar una herramienta, skill o agente externo
sin que exista un contrato formal (schema de input/output, trust_tier, version).
Capas que deben cumplir P13: CAPA_3, CAPA_4, CAPA_7, CAPA_12.
Referencia: RES.117 (ToolRegistry), RES.118 (SkillValidationPipeline).
...
## PRINCIPIOS NUEVOS EN V4 — PATCH P14P15 (RES.127)

**P14 — RBAC ownership. [NUEVO V4 — RES.127]**
El modelo de permisos de tenants sobre tools y skills pertenece
exclusivamente a CAPA_9. Ninguna capa de transporte (CAPA_2) implementa
logica de permisos de negocio. Toda consulta RBAC pasa por CAPA_9.rbac_check().
INV-P14: toda llamada a rbac_check() en cualquier capa DEBE pasar por CAPA_9.rbac_check().
Referencia: INC-02 / RES.136 / RES.127.

**P15 — Aislamiento de sesion es configurable, no implicito. [NUEVO V4 — RES.127]**
El mecanismo de aislamiento (UnikerManager vs SandboxManager) se selecciona via
isolation_mode en policy.yaml. Son mutuamente excluyentes por sesion.
Default: UnikerManager (unikernel per-tenant). Alternativa: SandboxManager (gVisor).
INV-ISO.1: exclusion mutua por sesion.
INV-ISO.2: default=unikernel (fail-secure).
INV-ISO.3: cambio de isolation_mode requiere restart del tenant.
Referencia: INC-04 / RES.138 / RES.127.
...
**Sin cambios V4**
Nota P14: CAPA_02 NO implementa logica de permisos de negocio — eso es exclusivo CAPA_09.
...
| CAPA_07 | Hereda V3_03 | Sin cambios. Nota: RBAC consumidor de CAPA_09 (P14) |
| **CAPA_08** | Modificada — RES.158 derivado | namespace hebbiano `mpat:dream:{tenant_id}:{session_id}:feedback` |
| CAPA_09 | Hereda V3_03 + P14 | **RBAC ownership exclusivo** — unico propietario de rbac_check() en el sistema |
| **CAPA_10** | Ampliada | OpenInference/QUIC integrado (RES.157 de V3) |
| CAPA_11 | Hereda V3_03 | Sin cambios. Nota: isolation_mode gestionado desde policy.yaml (P15) |
...
### Nota especial CAPA_09 — Impacto de P14

Con la adicion de P14, CAPA_09 es ahora el UNICO propietario de la logica RBAC.
Cualquier capa que necesite verificar permisos de tenant sobre tools/skills
DEBE llamar a CAPA_09.rbac_check() — no implementar logica propia.
Esto afecta especialmente a CAPA_02 (transporte), CAPA_07 (herramientas) y CAPA_12 (A2A).

### Nota especial CAPA_11 — Impacto de P15

Con la adicion de P15, el mecanismo de aislamiento de sesion se configura
exclusivamente via isolation_mode en policy.yaml (CAPA_14, Nivel 1).
CAPA_11 implementa ambos mecanismos (UnikerManager y SandboxManager) pero
no decide cual usar — esa decision pertenece a policy.yaml + P15.
...
[CAPA 7] Skills/MCP 2.0/A2A — herramientas (P13: contrato antes de invocar)
         (RBAC: consumidor de CAPA_09.rbac_check() — P14)
 |
...
[CAPA 9] Critic/Security — NHP Protocol, ASL-3, ZTS, HITL, Semantic Firewall
         **RBAC ownership exclusivo (P14) — unico propietario de rbac_check()**
 |
...
Transversales (todas las capas los usan):
 ...
 [CAPA 11] Workers — Redis, RQ, auto-scaling, Unikernel, SubQ
           (isolation_mode desde policy.yaml — P15)
...
RBAC:       CAPA_09 como unico propietario de permisos (P14 — V4 nuevo)
Aislamiento: isolation_mode configurable via policy.yaml (P15 — V4 nuevo)
```
...
| PATCH P14P15 | Principios nuevos | P14 (RBAC ownership) + P15 (aislamiento configurable) agregados | ariel.garcia.traba@gmail.com | 2026-05-23 |
| DT-ARQ-01 | Patch arquitectura | P14 + P15 integrados en ARQUITECTURA_base_V4 (este archivo) | Claude Sonnet 4.6 | 2026-05-29 |

---

## CONCILIACION DE FUENTES — DT-ARQ-01

| Fuente | Estado P14/P15 | Fecha | Evidencia | Confianza |
|---|---|---|---|---|
| ARQUITECTURA_base_V4_COMPLETA_V4_migrado.md (ariel.garcia.traba) | P14 + P15 completos con INV, referencias a RES.127, INC-02, INC-04, RES.136, RES.138 | 2026-05-23 | Texto canonico completo con invariantes INV-P14, INV-ISO.1/2/3 | ALTA |
| ARQUITECTURA_base_V4.md (ID: 1cyg9BLUeSi4VUJWVVpvVxG_j2JOMeuX9 — agt1973) | Sin P14, sin P15 — dice "13 principios" | 2026-05-22 | Archivo original RELAY_033 — anterior al PATCH P14P15 | MEDIA (version anterior) |
| ARQUITECTURA_base_V4_UNIF.md (ID: 1JlZPRoYVk33jy51Z_pk2yIFEP5wUrtnq) | Menciona P14+P15 en indice de cambios del frontmatter | 2026-05-24 | Confirma que el PATCH P14P15 existe y aplica | MEDIA (snippet) |

**Razonamiento:** El archivo objetivo es la version original del RELAY_033 (2026-05-22), previo al PATCH P14P15 aplicado el 2026-05-23. El texto de P14 y P15 en ARQUITECTURA_base_V4_COMPLETA_V4_migrado.md es la fuente canonica — contiene invariantes completos, referencias a INC-02, INC-04, RES.127, RES.136, RES.138. No hay conflicto de contenido entre fuentes — solo ausencia en el archivo objetivo.

**Decision:** Contenido de P14 y P15 tomado de ARQUITECTURA_base_V4_COMPLETA_V4_migrado.md sin modificacion.
**Estado:** RESUELTO — DT-ARQ-01 CERRADA.

---

*ARQUITECTURA_base_V4_V2.md · DT-ARQ-01 · Claude Sonnet 4.6 · 2026-05-29*
*Fuente original: ID 1cyg9BLUeSi4VUJWVVpvVxG_j2JOMeuX9 (RELAY_033 · agt1973@gmail.com · 2026-05-22)*
*P14/P15 fuente: ARQUITECTURA_base_V4_COMPLETA_V4_migrado.md (ariel.garcia.traba@gmail.com · 2026-05-23)*
*que has usado el formato de razonamiento adaptado por AGT_2026*
