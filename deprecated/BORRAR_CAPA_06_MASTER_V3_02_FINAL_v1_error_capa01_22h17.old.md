...
- NO gestiona aislamiento unikernel -> CAPA_01
...
> CORRECCION APLICADA EN MERGE: Fuente A indicaba CAPA_11 para unikernel.
> Fuente B + INFORME coinciden en CAPA_01 (TenantUnikernelRouter). Se usa CAPA_01.
...
Invariante que la cierra: INV-6-MEA.1 — suma de weights == 1.0 (+-0.001). Con muchos perfiles de peso
similar, el consenso pierde diferenciacion por dimension.
...
(temperatura alta + respuestas largas correlacionan positivamente para este usuario). CAPA_08 refuerza
conexiones entre **fragmentos de memoria del agente** (el fragmento A y el fragmento B se activan juntos
frecuentemente). Mismo principio de Hebb, implementaciones y propositos incompatibles.
Invariante que la cierra: INV-6-DC.3 — DreamConsolidator.run() es idempotente por session_id; el
HebbianReinforcer de CAPA_06 opera sobre el UserModel, no sobre los fragmentos de CAPA_08.
...
> DT-06-01 CERRADA por RES.158 (2026-05-22): namespace corregido de
> mpat:cx:{session_id}:experts a mpat:cx:{tenant_id}:{session_id}:experts.
> INV-158.1 aplica: ningun dato de tenant A puede aparecer en namespace de tenant B.
...
- Fuente A indicaba "CAPA_11" para unikernel isolation.
- Fuente B + INFORME + CAPA_01_MASTER indican "CAPA_01" (TenantUnikernelRouter).
- Resolucion: CAPA_01 — mayoria 2:1 + evidencia directa del componente en CAPA_01.

Aporte exclusivo de cada fuente:
- Fuente A: componentes RLHF/MEA/Dream COMPLETOS con DbC, 4 trampas completas, config policy.yaml, flujo de datos
- Fuente B: NOTA PEDAGOGICA DE APERTURA, numeracion de secciones, ESTADO AL CIERRE
